...
namespace {
    ...
    TEST_F(DBClientTest, CountWithHint) {
        ...
        ASSERT_EQUALS(c.count(TEST_NS, Query("{'a': 1}"), 0, 0, 0, "_id_"), 1U);
        ASSERT_EQUALS(c.count(TEST_NS, Query(), 0, 0, 0, "_id_"), 2U);

        IndexSpec sparse_spec;
        sparse_spec.addKey("b").sparse(true);
        c.createIndex(TEST_NS, sparse_spec);

        ASSERT_EQUALS(c.count(TEST_NS, Query("{'a': 1}"), 0, 0, 0, "b_1"), 0U);
        ASSERT_EQUALS(c.count(TEST_NS, Query(), 0, 0, 0, "b_1"), 2U);
    }
